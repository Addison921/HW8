...
class Pump_Controller():
    ...
    def ImportFromFile(self, data):
        ...
        :return: 
        """
        # data[0] is the pump name line
        self.Model.PumpName = data[0].strip()

        #data[1] is the column headers ("flow head efficiency")
...
class Pump_View():
    ...
    def DoPlot(self, Model):
        ...
        axes = self.ax
        axes.clear() #(many lines to make the graph)

        # Title and X-axis label
        axes.set_title(Model.PumpName, color='black')
        axes.set_xlabel(f"Flow ({Model.FlowUnits})", color='black')

        # Left y-axis for Head
        axes.set_ylabel(f"Head ({Model.HeadUnits})", color='black')
        # Plot raw Head data: black outlined circle (white inside)
        axes.plot(Model.FlowData, Model.HeadData, marker='o', linestyle='none',
                  markerfacecolor='white', markeredgecolor='black', label='Head Data')
        # Plot fitted Head curve: black dashed line
        axes.plot(headx, heady, linestyle='--', color='black',
                  label=f'Head Fit (R^2={headRSq:.3f})')

        # Right y-axis for Efficiency
        ax2 = axes.twinx()
        ax2.set_ylabel("Efficiency (%)", color='black')
        # Plot raw Efficiency data: black outlined triangle (white inside)
        ax2.plot(Model.FlowData, Model.EffData, marker='^', linestyle='none',
                 markerfacecolor='white', markeredgecolor='black', label='Eff Data')
        # Plot fitted Efficiency curve: black dotted line
        ax2.plot(effx, effy, linestyle=':', color='black',
                 label=f'Eff Fit (R^2={effRSq:.3f})')
