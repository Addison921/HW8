...
class rankineView():
    ...
    def selectQualityOrTHigh(self, Model=None):
        ...
        if self.rdo_Quality.isChecked():
            ...
            # "T High" is selected; update turbine inlet to the saturation temperature at PHigh.
            if Model is not None:
                from UnitConversions import UnitConverter as UC
                satHigh = Model.steam.getsatProps_p(Model.p_high)
                T_sat = satHigh.tsat
                if not self.rb_SI.isChecked():
    ...
    def setNewPHigh(self, Model=None):
        ...
        from UnitConversions import UnitConverter as UC
        SI = self.rb_SI.isChecked()
        # If p_high is not set yet, update it from the UI.
        if Model.p_high is None:
            try:
                Model.p_high = float(self.le_PHigh.text()) * (1 if SI else UC.psi_to_bar)
            except:
                Model.p_high = 80.0  # fallback default value
        satHigh = Model.steam.getsatProps_p(Model.p_high)
        self.lbl_SatPropHigh.setText(satHigh.getTextOutput(SI=SI))
        # Instead of checking rdo_THigh (which does not exist), check if Quality is not selected.
    ...
    def setNewPLow(self, Model=None):
        """
            Update the label for the low-pressure saturated properties.
        """
        from UnitConversions import UnitConverter as UC
        SI = self.rb_SI.isChecked()
        if Model.p_low is None:
            try:
                Model.p_low = float(self.le_PLow.text()) * (1 if SI else UC.psi_to_bar)
            except:
                Model.p_low = 0.08  # fallback default value
        satLow = Model.steam.getsatProps_p(Model.p_low)
        self.lbl_SatPropLow.setText(satLow.getTextOutput(SI=SI))
    ...
    def updateUnits(self, Model=None):
        ...
        from UnitConversions import UnitConverter as UC
        SI = Model.SI
        # Step 0: Update outputs.
        self.outputToGUI(Model=Model)
        # Step 1: Update pressures in the input fields.
        ...
        # Step 2: Update turbine inlet condition if T High is selected.
        if not self.rdo_Quality.isChecked():
            T_sat = Model.steam.getsatProps_p(Model.p_high).tsat
            if not SI:
                T_sat = UC.C_to_F(T_sat)
            self.le_TurbineInletCondition.setText("{:.2f}".format(T_sat))
            self.le_TurbineInletCondition.setEnabled(False)
        # Step 3: Update the label texts for pressures.
        if SI:
            self.lbl_PHigh.setText("P High (bar)")
            self.lbl_PLow.setText("P Low (bar)")
        else:
            self.lbl_PHigh.setText("P High (psi)")
            self.lbl_PLow.setText("P Low (psi)")
